<script lang="ts">
    ...
    const addUser = async () => {
        ...
        return fetch('http://localhost:8000/api/users', {
            ...
            if (!response.ok) {
                const error = (data && data.message) || response.status === 400? $_('Error.InvalidInput'): $_('Error.General');
...
<form>
    ...
    <div>
        <Button class="action-button" on:click={onClickSave} disabled={username.length===0||invalid||password.length===0}>
            <Icon>
                <ContentSave />
            </Icon>
