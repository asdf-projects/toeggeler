...
<div class="users-with-stats">
	<div class="user-list">
		<List class="user-list" twoLine avatarList singleSelection>
			{#await loadUsers() then registeredUsers}
				{#each registeredUsers as user}
					<Item>
						<Graphic
							style="background-image: url(https://place-hold.it/40x40?text={user.id}&fontsize=16);"
						/>
						<Text>
	...
	<div class="user-stats">
		{#if selectedUser}
			{#await loadUserStatistics(selectedUser.id) then stats}
				<h3>{$_('Users.StatsFor')} {selectedUser.username}</h3>
				<List>
					<Item>
						<Text>
							{$_('Users.Wins')}: {stats.wins} / {stats.losses} ({getWinLossRatio(
								stats.wins,
								stats.losses
							)}%)
						</Text>
